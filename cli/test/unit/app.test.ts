...
describe('runApp', () => {
  beforeEach(() => {
    ...
    // This is a dependency for createTemplate/updateTemplate within runApp
        vi.spyOn(FileService, 'getRootDir').mockReturnValue('D:/templates')
      })
  ...
  it('should handle the `sync` command', async () => {
    ...
    await runApp()
    expect(FileService.syncConfig).toHaveBeenCalledWith('my-config')
