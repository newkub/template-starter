...
describe('FileService.syncConfig', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    // Spy on methods within the FileService to isolate the logic of syncConfig
    vi.spyOn(FileService, 'getRootDir').mockReturnValue('/fake/root')
  ...
  it('should sync matching files', async () => {
    ...
    expect(existsSpy).toHaveBeenCalledWith('/fake/root/templates/my-config')
    expect(readdirSpy).toHaveBeenCalledTimes(2)
    expect(FileService.copyFile).toHaveBeenCalledWith('/fake/root/templates/my-config/config.json', '/fake/current/config.json')
