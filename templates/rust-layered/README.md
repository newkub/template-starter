...
**A Rust library template with Layered Architecture, Tokio, and cargo-nextest**

A starting point for medium-scale Rust crates that prefer a traditional layered layout to vertical slices. The `Cargo.toml` declares a `library` crate with the same task chain as the Clean Architecture template (`dev`, `format`, `lint`, `typecheck`, `test`, `build`, `verify`, `ci`), exposed through `[package.metadata.scripts]` and wired into the monorepo by `moon.yml`.
...
|------|---------|-------------|---------|-------|
| <img src="https://api.iconify.design/lucide:layers.svg?color=%23dea584" width="18" height="18"> | **Layered Architecture** | Technical layers with clear responsibilities | Easy to onboard, easy to slot into existing layouts | `src/modules/`, `src/adapters/` |
| <img src="https://api.iconify.design/lucide:zap.svg?color=%23f59e0b" width="18" height="18"> | **Feature Modules** | Feature-specific logic under `src/modules/<feature>/` | Keep each feature self-contained | `src/modules/booking/` |
...
> [!NOTE]
> How Layered Architecture is laid out in this template
...
|------|---------|---------|
| <img src="https://api.iconify.design/lucide:layout.svg?color=%236366f1" width="18" height="18"> | **Layered Architecture** | Each layer has a single responsibility and a single direction of dependency |
| <img src="https://api.iconify.design/lucide:shield.svg?color=%238b5cf6" width="18" height="18"> | **Pure Domain** | Business rules with no I/O and no third-party crates |
...
| <img src="https://api.iconify.design/lucide:box.svg?color=%23f59e0b" width="18" height="18"> | **Adapters** | Real implementations of the ports the domain defines |
...
| Question | Answer |
|----------|--------|
| What is Layered Architecture? | Organising code into layers (presentation, application, domain, adapters) where each layer has a single responsibility |
| How does this differ from the Clean Architecture template? | The folder layout is the same; the difference is the framing — slices (features) versus layers (technical concerns) |
...
1. **Scaffold the crate into a fresh directory**
   ```bash
   templates use rust-layered -o ./my-crate
   cd my-crate
   ```
...
moon run rust-layered:verify

# Run typecheck and tests
moon run rust-layered:typecheck
moon run rust-layered:test

# Format, lint, and build
moon run rust-layered:format
moon run rust-layered:lint
moon run rust-layered:build

# Watch for changes
moon run rust-layered:watch
```
...
| Application | Use cases, orchestration | Domain, `crate::shared::*` |
